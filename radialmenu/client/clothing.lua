...
local Drawables = {
	...
	["Shoes"] = {
		Drawable = 6,
		Table = {Standalone = true, Male = 34, Female = 63},
...
RegisterNetEvent('radialmenu:ResetClothing')
AddEventHandler('radialmenu:ResetClothing', ResetClothing)
...
function ToggleClothing(whic, extra)
	...
	local Ped = PlayerPedId()
	--print(GetPedDrawableVariation(Ped, Toggle.Drawable))
	...
	if which ~= "Mask" then
		if not Gender then Notify("Tento Ped s tímto nefunguje") return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify("Nic neexistuje pro tuto možnost") return
	else
		if not LastEquipped[which] then
...
RegisterNetEvent('radialmenu:ToggleClothing')
AddEventHandler('radialmenu:ToggleClothing', ToggleClothing)
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify("Nemáš to na sobě") return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then Notify("Tento Ped s tímto nefunguje") return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify("Žádná možnost") return false
	end
end
...
RegisterNetEvent('radialmenu:ToggleProps')
AddEventHandler('radialmenu:ToggleProps', ToggleProps)
--print(ToggleProps)
